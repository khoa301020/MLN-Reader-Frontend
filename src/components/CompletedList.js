      ...
      <div className="mx-auto max-w-2xl lg:max-w-7xl">
        <div className="grid grid-cols-1 gap-y-1 gap-x-6 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 xl:gap-x-8">
          {bookInfos.map((bookInfo) => (
            <a key={bookInfo.id} href={bookInfo.href} className='group no-underline' >
              ...
              <h3 className="mt-1 text-sm text-gray-700 text-center hover:opacity-75 line-clamp-2">{bookInfo.name}</h3>
            </a>
          ))}
        </div>
      </div>
