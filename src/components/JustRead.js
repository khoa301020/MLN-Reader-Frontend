        ...
        {history &&
          history.map((item, index) => (
            <div className="flex flex-col mb-2" key={index}>
              <div className="flex">
                <a
                  href={`/${item.type}/${item.type === 'novel' ? item.novelId : item.mangaId}`}
                  className="hover:opacity-75"
                >
                  <img
                    src={item.type === 'novel' ? item.novelCover : item.mangaCover}
                    className="object-cover w-16 h-24 rounded-sm"
                    alt=""
                  />
                </a>
                <div className="flex flex-col pl-3 max-h-24 w-full">
                  <a
                    href={`/${item.type}/${item.type === 'novel' ? item.novelId : item.mangaId}`}
                    className="no-underline text-cyan-500 hover:text-cyan-400"
                  >
                    <div className="font-bold line-clamp-1 space-x-3 mb-1">
                      {item.type === 'novel' ? item.novelTitle : item.mangaTitle}
                    </div>
                  </a>
                  <a
                    href={`/${item.type}-chapter/${item.chapterId}`}
                    className="no-underline text-xs text-zinc-300 hover:text-zinc-100"
                  >
